(() => {
  ...
  // ---- Tema (usado por el botón del HTML) ----
  ...
  function buildMenu() {
    ...
    // Si en el JSON hay "category", agrupamos; si no, lista plana
    const byCat = new Map();
    ...
    for (const [cat, items] of byCat) {
      if (byCat.size > 1) {
        ...
        h.innerHTML = `<div class="sidebar-title">${cat}</div>`;
        frag.appendChild(h);
      }
    ...
    menu.addEventListener("click", e => {
      ...
      if (btn) loadTopic(btn.dataset.id);
    });
  }

  // ---- Modo lectura (oculta/mostrar sidebar) ----
  function addReaderToggle() {
    const css = `
      .reader-toggle{position:fixed;bottom:1rem;right:1rem;z-index:9999;border:0;border-radius:999px;padding:.65rem .95rem;font-weight:600;cursor:pointer;background:linear-gradient(135deg,var(--accent-2),var(--accent-4));color:#0b0d12;box-shadow:0 10px 20px rgba(0,0,0,.25)}
  ...
  window.addEventListener("load", async () => {
    try {
      ...
      buildMenu();
      addReaderToggle();
      // Si quieres abrir una técnica por defecto, descomenta:
      // loadTopic(TOPICS[0]?.id);
    } catch (err) {
      console.error(err);
      $("#topicContent").innerHTML = "<p>No se pudo cargar el contenido. Verifica <code>topics.json</code>.</p>";
    }
  });
})();
